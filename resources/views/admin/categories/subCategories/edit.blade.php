            ...
            <form action="{{ route('subcategoryupdate',$subcategory->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        ...
                        <input type="text" name="name" value="{{ old('name',$subcategory->name) }}" class="form-control" placeholder="Name">
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select class="custom-select" name="parent_id">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ $category->id == $subcategory->parent_id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                        <div class="clearfix"></div>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <select class="custom-select" name="status">
                            ...
                            <option value="Active" {{ $subcategory->status == 'Active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                            <option value="Inactive" {{ $subcategory->status == 'Inactive' ? 'selected' : '' }}>I{{ __('Inactive') }}</option>
                        </select>
                        <div class="clearfix"></div>
